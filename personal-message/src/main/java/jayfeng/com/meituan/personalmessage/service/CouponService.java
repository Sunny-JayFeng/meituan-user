...
public class CouponService {
    ...
    public ResponseData getAllValidCoupons(Integer userId) {
        ...
        logger.info("getAllValidCoupons 获取用户有效优惠券成功 userId: {}, size: {}", userId, couponList.size());
    ...
    public ResponseData getAllUnValidCoupons(Integer userId) {
        ...
        logger.info("getAllUnValidCoupons 获取用户失效优惠券成功 userId: {}, size: {}", userId, couponList.size());
    ...
    public ResponseData useCoupon(Integer couponId) {
        ...
        if (coupon == null || coupon.getStatus().equals(UN_VALID_STATUS)) {
            logger.info("useCoupon 优惠券已失效, 使用失败 coupon: {}", coupon);
        ...
        logger.info("useCoupon 优惠券使用成功 coupon: {}", coupon);
    ...
    public void batchSetCouponUnValid(List<Coupon> couponList) {
        logger.info("batchSetCouponUnValid 批量设置优惠券失效, size: {}", couponList.size());
        for (Coupon coupon : couponList) {
            logger.info("batchSetCouponUnValid 优惠券失效, coupon: {}", coupon);
    ...
    public ResponseData addCoupon(Coupon coupon) {
        if (coupon == null) {
            logger.info("addCoupon 优惠券为空, 添加失败");
        ...
        if (ObjectUtils.isEmpty(userId) || userDao.selectIdById(userId) == null) {
            logger.info("addCoupon 用户不存在, 优惠券添加失败 userId: {}", userId);
        ...
        logger.info("addCoupon 优惠券添加成功 coupon: {}", coupon);
    ...
    public ResponseData removeOneUnValidCoupon(Integer couponId) {
        Coupon coupon = couponDao.findCouponById(couponId);
        logger.info("removeOneUnValidCoupon 删除一张失效优惠券, coupon: {}", coupon);
    ...
    public ResponseData removeAllUnValidCoupons(Integer userId) {
        List<Coupon> couponList = couponDao.findAllCouponByUserId(userId, UN_VALID_STATUS);
        logger.info("removeAllUnValidCoupons 清空失效优惠券, size: {}", couponList.size());
